...
class InsertTest(unittest.TestCase):
    ...
    def test_insert_unbalanced_case1(self):
        ...
        for key in keys:
            self.tree.insert(key, str(key))
        # self.tree.print_tree()
        self.assertEqual(1, self.tree.height, "Tree height error")
    ...
    def test_insert_unbalanced(self):
        ...
        for key in keys:
            self.tree.insert(key, str(key))
        self.assertEqual(4, self.tree.height, "Tree height error")
...
class DeleteUnbalancedTest(unittest.TestCase):
    ...
    def test_case1a(self):
        ...
        self.tree.delete(node)
        # self.tree.print_tree()
        self.assertEqual(1, self.tree.height)
        self.assertEqual(8, self.tree.max_node().key, "Max node error")
...
class JoinTest(unittest.TestCase):
    def test_join(self):
        ...
        tree2 = AVLTree()
        for key in keys1:
            tree1.insert(key, str(key))
        for key in keys2:
            tree2.insert(key, str(key))
        # tree1.print_tree()
        # tree2.print_tree()
        tree_height = tree1.height
        median_key = 9
        median_value = str(median_key)
        tree1.join(tree2, median_key, median_value)
        # tree1.print_tree()
        self.assertEqual(tree1.size(), len(keys1) + len(keys2) + 1)
        for key in keys1 + keys2 + [median_key]:
            node, _ = tree1.search(key)
            self.assertIsNotNone(node, f"Key {key} not found after join")
            self.assertEqual(key, node.key)
        expected_size = len(keys1) + len(keys2) + 1
        self.assertEqual(expected_size, tree1.size())
        diff = tree1.height - tree_height
...
class SplitTest(unittest.TestCase):
    ...
    @unittest.skip("Skipping test temporarily")
    def test_split(self):
        split_key = 20
        left_tree, right_tree = self.tree.split(split_key)
        self.assertEqual(left_tree.size(), 3)
        self.assertEqual(right_tree.size(), 3)
        left_keys = [10, 5, 15]
        right_keys = [30, 25, 35]
        for key in left_keys:
            node, length = left_tree.search(key)
            self.assertIsNotNone(node)
            self.assertEqual(key, node.key)
        for key in right_keys:
            node, length = right_tree.search(key)
            self.assertIsNotNone(node)
            self.assertEqual(key, node.key)
...
class AVLToArrayTest(unittest.TestCase):
    ...
    def test_avl_to_array(self):
        ...
        self.assertEqual(len(arr), len(expected_keys))
        for i, key in enumerate(expected_keys):
            self.assertEqual(key, arr[i].key)
